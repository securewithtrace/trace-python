...
from securewithtrace.webhooks.events import TraceEvent, parse_event
...
class WebhookHandler:
    secret: str
    tolerance: timedelta = field(default=_DEFAULT_TOLERANCE)
    ...
    def verify_and_parse(self, payload: bytes, signature: str, event_type: str) -> TraceEvent:
        ...
        event = parse_event(event_type=event_type, data=parsed_payload)

        now = datetime.now(timezone.utc)
        if abs(now - event.created_at) > self.tolerance:
            raise WebhookVerificationError(
                f"Event timestamp is outside the allowed tolerance of {self.tolerance}."
            )
